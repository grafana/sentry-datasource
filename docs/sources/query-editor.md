...
| **Dataset**        | (Optional) The Sentry dataset to query: `errors` or `transactions`. When unset, Sentry queries its default Discover dataset, as in earlier plugin versions.                                                                                                  |
...
| **Sort By** | (Optional) The field to sort grouped results by, such as an aggregate like `-count()` or a **Group** field.                                                                                                                         |
...
- **Error rate by release:** Set **Y-axis** to `count()`, **Query** to `event.type:error`, **Group** to `release`, and **Sort By** to `release` to compare error rates across releases.
...
| **Y-axis**  | (Required) One or more fields to plot on the y-axis. Enter field names and press Enter to add each one.                                                          |
| **Group**   | (Optional) One or more fields or tags to group results by. Enter values and press Enter to add each one.                                                         |
| **Sort By** | (Required when **Group** is set) The field to sort grouped results by. Accepts **Group** fields and, unlike Events Stats, aggregate functions such as `count()`. |
| **Limit**   | (Optional) The maximum number of result groups. Maximum: `10`, a cap imposed by the Sentry API.                                                                  |
...
- **Slow HTTP calls by endpoint:** Set **Y-axis** to `avg(span.duration)`, **Query** to `span.op:http.client`, **Group** to `span.description`, and **Sort By** to `avg(span.duration)` to identify slow external calls.

### Release Health (Sessions)

Use the Release Health (Sessions) query type to retrieve session-based release health data from Sentry, such as crash rates, crash-free rates, and session counts.

| Field        | Description                                                                                                                                                                                                                                       |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Field**    | (Required) The session field to query. Options: `sum(session)`, `count_unique(user)`, `crash_free_rate(session)`, `crash_free_rate(user)`, `crash_rate(session)`, `crash_rate(user)`, `anr_rate()`, `foreground_anr_rate()`.                      |
| **Query**    | (Optional) A Sentry search query to filter results, such as `session.status:crashed`. Run with Shift+Enter.                                                                                                                                       |
| **Group By** | (Optional) Group results by `environment`, `project`, `session.status`, or `release`.                                                                                                                                                             |
| **Sort By**  | (Optional) Sort grouped results by a session field. Appears only when a Group By value other than `session.status` is set.                                                                                                                        |
| **Order**    | (Optional) Sort order: High to low or Low to high. Appears alongside Sort By.                                                                                                                                                                     |
| **Limit**    | (Optional) The maximum number of result groups. Default: `5`, maximum: `100`. Appears alongside Sort By. On dense time ranges the plugin widens the interval and lowers the limit to keep the query within the Sentry API's response size limits. |

There are no per-status fields such as `session.crashed` or `session.healthy`. To break sessions or users down by status, set **Field** to `sum(session)` or `count_unique(user)` and **Group By** to `session.status`. To count only sessions with a single status, use a query filter such as `session.status:crashed` instead.

#### Migrating from the Metrics query type

Queries saved with plugin versions before 3.0.0 used the Metrics query type name with different field names. The query type value is unchanged, so saved queries keep working once their **Field** and **Sort By** selections are re-selected using the new names:

| Field before 3.0.0                                                                                                            | Field from 3.0.0                                                                                                                      |
| ----------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| `session.all`                                                                                                                 | `sum(session)`                                                                                                                        |
...
| `session.anr_rate`                                                                                                            | `anr_rate()`                                                                                                                          |
| `session.foreground_anr_rate`                                                                                                 | `foreground_anr_rate()`                                                                                                               |
| Per-status fields (`session.crashed`, `session.errored`, `session.healthy`, `session.abnormal`, and their `_user` variants)   | `sum(session)` or `count_unique(user)` with **Group By** set to `session.status`, or a query filter such as `session.status:crashed`  |

#### Release Health use cases
