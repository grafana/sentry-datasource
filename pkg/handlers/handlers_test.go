...
func TestSentryDatasource_Spans(t *testing.T) {
	t.Run("valid spans query should produce correct result", func(t *testing.T) {
		sc := util.NewFakeClient(util.FakeDoer{Body: `{
			"data": [
				{
					"id": "event_id_1",
		...
		assert.Nil(t, res.Responses["A"].Error)
		require.Equal(t, 1, len(res.Responses["A"].Frames))
		assert.Equal(t, "Spans (A)", res.Responses["A"].Frames[0].Name)
