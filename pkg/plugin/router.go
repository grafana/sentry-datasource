...
func (ds *SentryDatasource) getResourceRouter() *mux.Router {
	...
	router.HandleFunc("/api/0/organizations/{organization_slug}/releases", ds.withDatasourceHandler(GetReleasesHandler)).Methods("GET")
...
func GetReleasesHandler(client *sentry.SentryClient) http.HandlerFunc {
	return func(rw http.ResponseWriter, r *http.Request) {
		orgSlug := mux.Vars(r)["organization_slug"]
		if orgSlug == "" {
			http.Error(rw, "invalid orgSlug", http.StatusBadRequest)
		...
		releases, err := client.ListOrganizationReleases(r.Context(), orgSlug, r.URL.Query()["project"])
		writeResponse(releases, err, rw)
	}
}
