...
func TestSentryClient_GetStatsV2(t *testing.T) {
	t.Run("interval handling", func(t *testing.T) {
		...
		tests := []struct {
			...
			{name: "fractional hour interval snaps up to two hours", interval: "1.5h", timeRange: 7 * 24 * time.Hour, wantInterval: "2h"},
			{name: "compound duration snaps up to three hours", interval: "2h30m", timeRange: 30 * 24 * time.Hour, wantInterval: "3h"},
			{name: "interval that does not divide a day snaps up", interval: "7m", timeRange: 6 * time.Hour, wantInterval: "10m"},
			{name: "interval below the bucket cap snaps up", interval: "1m", timeRange: 24 * time.Hour, wantInterval: "2m"},
			{name: "hour interval over sixty days snaps up for the bucket cap", interval: "1h", timeRange: 60 * 24 * time.Hour, wantInterval: "2h"},
